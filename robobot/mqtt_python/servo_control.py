...
class Servo:
    ...
    def set_position(self, value: int, speed: int | None = None) -> bool:
        ...
        return service.send("robobot/cmd/T0", f"servo {self.servo_no} {target} {use_speed}")

    def get_preset_value(self, name: str) -> int:
        """Get a named preset value for this class."""
        if name not in self.PRESETS:
            raise KeyError(f"Preset '{name}' not configured for {self.__class__.__name__}")
...
class Gripper(Servo):
    ...
    def close(self, speed: int | None = None) -> bool:
        """Move to close preset using instance defaults unless overridden."""
        return self.set_position(self.get_preset_value("close"), speed=speed)
