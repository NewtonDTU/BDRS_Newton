...
def drive_until_line(speed=0.2, distance=0.0, recovery=False):
    ...
    while edge.lineValidCnt <= 4 and not service.stop:
        ...
        if use_distance_limit and abs(pose.tripB) >= target_distance:
            print(
                f"Distance limit reached ({target_distance:.3f} m) without line detection"
            )
            break
...
def drive_distance(distance=0.5, speed=0.2):
    ...
    while not (service.stop):
        ...
        if state == 0:
            ...
            print(
                f"# drive_to_wall reached {pose.tripB:.3f} m in {pose.tripBtimePassed():.3f} seconds"
            )
            service.send("robobot/cmd/ti", "rc 0 0")
            t.sleep(0.2)
            service.send("robobot/cmd/ti", "rc 0 0")
...
def driveTurn(angle=np.pi / 4, angular_speed=0.5):
    ...
    while not (service.stop):
        ...
        if state == 0:
            ...
            print(
                f"# turn45 turned {pose.tripBh:.3f} rad in {pose.tripBtimePassed():.3f} seconds"
            )
            service.send("robobot/cmd/ti", "rc 0.0 0.0")
            break
...
def driveCircle(rotation=2 * np.pi, v=0.2, w=0.6):
    ...
    while not (service.stop):
        ...
        if state == 0:  # start moving
            ...
            print(
                f"# circle completed {pose.tripBh:.3f} rad in {pose.tripBtimePassed():.3f} seconds"
            )
            service.send("robobot/cmd/ti", "rc 0.0 0.0")
            break

        print(
            f"# state {state}, heading {pose.tripBh:.3f} rad, time {pose.tripBtimePassed():.3f}"
        )
        t.sleep(0.05)
...
def follow_line_for_distance(
    distance=1.0, velocity=0.15, mode="center", ref_position=0.0
):
    """
...
def count_active_line_sensors(
    active_threshold=None, use_dynamic_threshold=True, dynamic_ratio=0.45
):
    """
...
def follow_line_until_intersection(
    ...
    loop_counter = 0
    while not service.stop:
        active_count, active_mask, active_indices, values, used_threshold, contrast = (
            count_active_line_sensors(
        ...
        crossing_ready = (
            edge.crossingLine and edge.crossingLineCnt >= min_crossing_count
        )
        active_ready = (
            active_count >= min_active_sensors and sensor_span >= min_sensor_span
        )
...
def follow_line_until_tilt_back_normal(
    ...
    with open("/tmp/ramp_debug.log", "w") as f:
        f.write(
            f"START: baseline_tilt={baseline_tilt:.4f}, tilt_up_limit={tilt_up_limit}, velocity={velocity}\n"
        )
        f.flush()
    ...
    while not service.stop and (t.time() - start < seconds):
        ...
        if debug_counter % 100 == 0:
            with open("/tmp/ramp_debug.log", "a") as f:
                f.write(
                    f"LOOP: tilt={pose.pose[3]:.4f}, delta={tilt_delta:.4f}, on_ramp={on_ramp}, elapsed={t.time()-start:.1f}s\n"
                )
                f.flush()
        ...
        if not on_ramp:
            if tilt_delta > tilt_up_limit:
                if tilt_up_start is None:
                    ...
                    with open("/tmp/ramp_debug.log", "a") as f:
                        f.write(
                            f"RISE: delta={tilt_delta:.4f} > limit={tilt_up_limit}\n"
                        )
                        f.flush()
...
def rotate(target_angle, turn_speed=0.5):
    ...
    while not service.stop:
        if state == 0:
            ...
            print(
                f"tripBh={pose.tripBh:.3f}, target={target_angle:.3f}, time={pose.tripBtimePassed():.2f}"
            )
...
def follow_line_until_angle(
    ...
    print(
        f"% Start line following, target={target_rad:.3f} rad ({np.rad2deg(target_rad):.1f} deg)"
    )
    ...
    while not service.stop:
        ...
        if check_rad >= target_check:
            print(
                f"% Target angle reached: {current_rad:.3f} rad ({np.rad2deg(current_rad):.1f} deg)"
            )
            if stop_on_reach:
...
def find_aruco_markers(frame, expected_marker_id=0):
    ...
    print(
        "X:{:.2f}m Y:{:.2f}m Z:{:.2f}m".format(
            marker_pos_ee[0], marker_pos_ee[1], marker_pos_ee[2]
        )
    )
...
def detect_luggages():
    try:
        ...
        print(f"An error occurred in function detect_luggages: {e}")
        cam.release()


def distance_from_marker(marker_id):
    cam = cv2.VideoCapture("http://10.197.216.163:7123/stream.mjpg")
    for i in range(3):
        ret, frame = cam.read()
        if not ret:
            continue
        marker_pos_ee20, _ = find_aruco_markers(frame, expected_marker_id=marker_id)
        if marker_pos_ee20 is not None:
            cam.release()
            return marker_pos_ee20[2]

        if i == 2:
            print(
                f"Could not detect marker ID {marker_id} after 3 attempts, returning None"
            )
            cam.release()
            return None
...
def go_to_marker_id(
    marker_id, distance_sp=0.4, dir=1, angle_tolerance=np.deg2rad(2), turn_speed=0.2
):
    start_time = t.time()
    ...
    while True:
        ...
        if elapsed_time > 120:
            print("Timeout reached while trying to find marker, stopping.")
            service.send("robobot/cmd/ti", "rc 0.0 0.0")
            return 0
        ...
        if marker_pos_ee is not None:
            marker_pos_ee[0] = marker_pos_ee[0] + 0.025  # offset
        ...
        if start_find_aruco:
            ...
            if find_aruco_state == 0:
                pose.tripBreset()
                print(
                    f"Turning robot by {np.rad2deg(turn_angle):.2f} degrees to face marker..."
                )
                find_aruco_state = 1
                ...
                if lost_marker == True:
                    find_aruco_state = 99
                angular_speed = Kp_turn * turn_angle * dir
                angular_speed = np.clip(angular_speed, -0.5, 0.5)
                service.send("robobot/cmd/ti", "rc 0.0 " + str(round(angular_speed, 2)))
                if abs(turn_angle) < angle_tolerance or pose.tripBtimePassed() > 20:
                    print(
                        f"Triggered by angle {np.rad2deg(turn_angle):.2f} degrees or timeout {pose.tripBtimePassed():.2f} seconds"
                    )
                ...
                if distance_to_marker < distance_sp:
                    ...
                    speed = 0.1
                    service.send(
                        "robobot/cmd/ti", "rc " + str(round(speed, 2)) + " 0.0"
                    )
                ...
                service.send("robobot/cmd/ti", "rc " + str(round(speed, 2)) + " 0.0")
                if (
                    distance_to_marker < distance_sp
                    or pose.tripBtimePassed() > 1
                    or lost_marker == True
                ):
                    print(
                        f"Triggered by distance {distance_to_marker:.2f} m or timeout {pose.tripBtimePassed():.2f} seconds"
                    )
                    service.send("robobot/cmd/ti", f"rc 0.0 0.0")
                    find_aruco_state = 8
                ...
                service.send(
                    "robobot/cmd/ti", f"rc 0.0" + str(round(turn_speed * dir, 2))
                )
...
def try_pickup(gripper, mainServo, id=None):

    gripper.open(200)
    if id is None:
        detected_marker_id, _ = detect_closest_luggage()
    else:
        detected_marker_id = id

    print(f"Detected marker ID: {detected_marker_id}")

    # If no luggage detected, retry
    if detected_marker_id is None:
        print("No luggage detected, retrying...")
        return None

    if (go_to_marker_id(detected_marker_id, 0.4, 1, turn_speed=0.1)):
        t.sleep(0.3)

        go_to_marker_id(detected_marker_id, 0.35, 1, turn_speed=0.1)
        drive_distance(0.08, 0.1)

        mainServo.goto_down()
        t.sleep(1)
        gripper.close_luggage(1200)
        t.sleep(1)
        mainServo.goto_semi_down()
        t.sleep(1)
        drive_distance(0.25, -0.1)

        return detected_marker_id
    else:
        return None
